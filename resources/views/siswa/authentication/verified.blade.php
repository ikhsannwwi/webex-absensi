...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('siswa.verified.verify', $uuid) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="mb-3 text-center">
                    ...
                    <input type="hidden" name="code_otp" id="code_otp">
                </div>
...
@push('js')
    <script>
        ...
        let resendButton = $("#triggerResend");
        let countdown;

        // Initialize an array to store the values of input fields
        let otpValues = Array(inputs.length).fill("");
        ...
        function addListener(input, index) {
            input.on("keyup", () => {
                ...
                // Update the value in the array and concatenate them
                otpValues[index] = input.val();
                const concatenatedValue = otpValues.join("");

                // Save the concatenated value into the hidden input
                $("#code_otp").val(concatenatedValue);
            });
        }
        ...
        resendButton.on("click", function() {
            // Disable the button
            resendButton.off("click", resendOtp);
            resendButton.css("pointer-events", "none");

            $.ajax({
                type: "GET",
                url: "{{ route('siswa.otp.send', $uuid) }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "_method": "GET",
                    "uuid": '{{ $uuid }}',
            ...
            countdown = setInterval(function() {
                if (seconds <= 0) {
                    ...
                    resendButton.on("click", resendOtp);
                    resendButton.css("pointer-events", "auto");
                    resendButton.text("Resend Otp Code");
                } else {
                    resendButton.text(`Resend Otp Code (${seconds}s)`);
                    seconds--;
                }
            }, 1000);

            // Call the resendOtp function (you can replace this with your actual resend logic)
            resendOtp();
        });
